...
const RepDetails = () => {
  ...
  useEffect(()=>{
    axios.get(`http://localhost:8000/api/rep/${id}`)
      .then(response=>{
    ...
    const handleSubmit = (e) => {
      e.preventDefault()
      axios.put(`http://localhost:8000/api/rep/${id}`, {firstName, lastName, email, office, address, phone, dob})
        .then(response=>{
          console.log(response.data)
          navigate('/')
        })
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:8000/api/rep/${id}`)
      .then(response=>{
